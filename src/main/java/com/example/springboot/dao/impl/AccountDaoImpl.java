...
public class AccountDaoImpl implements IAccountDao {
    ...
    public int add(Account account) {
        return jdbcTemplate.update("insert into account(name,money,createDate) values(?,?,?)",
                account.getName(),account.getMoney(),account.getCreateDate());
    }

    @Override
    public int update(Account account) {
        return jdbcTemplate.update("UPDATE account SET NAME = ? ,money=? , createDate=? where id=?",
                account.getName(),account.getMoney(),account.getCreateDate(),account.getId());
    }
